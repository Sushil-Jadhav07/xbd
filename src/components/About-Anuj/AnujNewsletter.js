...
const AnujNewsletter = ({ anujNewsletterData }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.error('Anuj newsletter subscription failed:', error)
      setMessage({
        type: 'error',
  ...
  const submitToMailchimp = (emailAddress) => {
    return new Promise((resolve, reject) => {
      ...
      const callbackName = `anujNewsletterCallback_${Date.now()}`
        ...
        <div className="text-center max-w-3xl mx-auto">
          ...
          >
            <input 
              ...
              required
              disabled={loading}
              className="flex-1 w-full sm:w-auto bg-gray-100 text-gray-700 px-6 py-3 rounded-lg border-none focus:ring-2 focus:ring-gray-400 focus:outline-none text-base disabled:bg-gray-200 disabled:cursor-not-allowed"
            />
            <button 
              type="submit"
              disabled={loading}
