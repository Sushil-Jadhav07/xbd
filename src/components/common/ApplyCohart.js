...
export default function ApplyCohartForm({ open, onClose, initialSubject = "" }) {
  ...
  function submitToMailchimp(values) {
    return new Promise((resolve, reject) => {
      ...
      const callbackName = "mcApplyCohartCallback_" + Date.now();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div className="relative z-10 w-full max-w-md bg-white rounded-2xl shadow-2xl p-6">
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          >
            {submitting ? "Submitting..." : "Apply Cohart"}
          </button>
        </form>
      </div>
    </div>
